...
def interp_wrapper(vol, coords, interptype="cubic"):
    ...
    if not np.any(np.isreal(vol)):
        ...
        transformeddata = map_coordinates(np.nan_to_num(np.real(vol)).astype(np.float64), coords, order=order, mode="nearest") + 1j * map_coordinates(
            np.nan_to_num(np.imag(vol)).astype(np.float64), coords, order=order, mode="nearest"
        )
        ...
        if interptype == "wta":
            ...
            for c_label in alllabels:
                allvols.append(map_coordinates(np.nan_to_num(vol == c_label).astype(np.float64), coords, order=order, mode="nearest"))
