...
def mapsurfacetovolume(data, vertices, res, specialmode, emptyval):
    ...
    for x_n in [-1, 1]:
        for y_n in [-1, 1]:
            for z_n in [-1, 1]:
                ...
                voxel_is = np.ravel_multi_index((x_r - 1, y_r - 1, z_r - 1), dims=(res, res, res), order="F")
                # 1 x vertices with the weight to assign
                voxel_w = (1 - x_d) + (1 - y_d) + (1 - z_d)

                # construct the entries and add the old one in
                x_new = sparse.coo_matrix((voxel_w, (vert_range, voxel_is)), shape=(n_vertices, n_voxels))
                x_new = x_old + x_new
                x_old = x_new
    ...
    if specialmode == 0:
        ...
        transformeddata = zerodiv(transformeddata, np.tile(wtssum, n_datasets), emptyval)

        # prepare the results
        transformeddata = np.reshape(transformeddata.T, [res, res, res, n_datasets], order="F")
        ...
        for data_q in np.arange(n_datasets):
            ...
            # save
            transformeddata.append(np.reshape(finaldata, [res, res, res], order="F"))
